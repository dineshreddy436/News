...
function setFieldError(fieldId, message) {
    ...
    if (field) {
        field.classList.toggle('form-error', !!message);
        const customSelect = field.closest('.custom-select');
        if (customSelect) customSelect.classList.toggle('is-error', !!message);
    }
...
function enhanceResponsiveSelects() {
    ...
    selects.forEach((select, index) => {
        ...
        menu.id = (select.id || 'select-' + index) + '-menu';
        trigger.setAttribute('aria-controls', menu.id);

        function updateValueText() {
            const selected = select.options[select.selectedIndex];
            valueText.textContent = selected ? selected.textContent : '';
            wrapper.classList.toggle('is-error', select.classList.contains('form-error'));
        ...
        if (formGroup) formGroup.classList.add('has-custom-select');
